...
use PHPMailer\PHPMailer\SMTP;
	...
	//
function sense_mail($emRem, $emUsr, $wise, $system_msg, $emAsu, $miMsg, $emIp) {
      global  $emRem, $emUsr, $wise, $system_msg, $emAsu, $miMsg, $emIp;
      $mail = new PHPMailer();
...
//        $mail->addCC('');
        ...
        $emMsg .= "Consulta a ".$wise." via @URLAPP \n";
        $emMsg .= $miMsg;
        $emMsg .= "\n";
        $mail->Body = $emMsg;
        ...
        $mail->send();
         ...
         $mailLog  = ">".$emIp."<< - >>".$emUsr."<< - >>".$miMsg." - ".$nowForm.PHP_EOL;
        file_put_contents('@NOMGPTMAILLOG', $mailLog, FILE_APPEND);
        echo "Email enviado !!".PHP_EOL;

      } catch (Exception $e) {

        $mailLog  = "ERROR >>>".$emIp."<< - >>".$emUsr."<< - >>".$miMsg." - ".$nowForm.PHP_EOL;
        file_put_contents('@NOMGPTMAILLOG', $mailLog, FILE_APPEND);
